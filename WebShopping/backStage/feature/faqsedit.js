// 宣告要帶入的欄位
let edit_sort = $('.edit_sort'), edit_tit = $('.edit_tit'), edit_titImg = $('.edit_titImg'), titImages = $('.titImages'), titImg = $('.titImg'), edit_brief = $('.edit_brief');
let btnBackOrg = $('.btnBackOrg'), orgImg, orgTop, orgSort, orgTitle, orgBrief, orgStatus, orgContent; // 排序不做使用：不做拋接
let edit_status = $('.edit_status');
let btnReturn = $('.btnReturn'), btnReset = $('.btnReset'), btnSaveFAQ = $('.btnSaveFAQ');

CONNECT = "KnowledgeContent";
// 接收資料，做渲染、處理
function process(data) {
    // 原資料
    orgImg = data.image_name + '?editz', orgTop = data.First, orgTitle = data.title, orgBrief = data.brief, orgStatus = data.Enabled, orgContent = data.content;
    // 置頂狀態
...
// 驗證
function dataUpdateCheck(aId, id, brief, contentz) {
    if (aId.trim() === '') {
        id.focus();
...
$().ready(function () {
    ...
    let num = localStorage.getItem('faqsNum');
    if (num) {
        //
        let dataObj = {
            "Methods": "GET",
    ...
    // Save 儲存
    btnSaveFAQ.on('click', function (e) {
        e.preventDefault();
        // 驗證
        dataUpdateCheck(idz, edit_tit, edit_brief, quill);
        if (check == true) {
            // 取得圖片的名稱包成陣列
