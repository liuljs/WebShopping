...
function process(data) {
    ...
    $('.btnEdit').on('click', function (e) {
        ...
        if (idz) {
            ...
            let faqsNum = $(this).parents('tr').find('.edit_num').data('num');
            localStorage.setItem("faqsNum", faqsNum);
            let numz = localStorage.getItem("faqsNum");
            if (numz) { // 確認有將消息編號存入 Storage
                location.href = "./faqsedit.html"; // 跳轉至編輯頁面
            };
        };
    });
    // Delete
...
$().ready(function () {
    // 清除 localStorag 中可能留著的 faqsNum
    if (localStorage.getItem('faqsNum')) {
        localStorage.removeItem('faqsNum');
    };
    //
    let dataObj = {
        "Methods": "GET",
